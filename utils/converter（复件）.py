...
def traj2text(trajectory: np.ndarray, prefix: str = "", suffix: str = ""):
    ...
    coord_text_list = [coord_prefix + coord_separator.join(
        [coord_template.format(trajectory[i, j]) for j in range(dim)]) + coord_suffix for i in range(frame)]
...
def text2traj(description: str, frame: int = 12, dim: int = 2):
    ...
    if len(description) == 0:
        error = True
        print("[text2traj parse failed]: Empty description")

    elif '[(' not in description or ')]' not in description:
        error = True
        print("[text2traj parse failed]: Missing '[(' or ')]'")
    ...
    if not error:
        try:
            ...
            description_list = list(eval(description_cleanup))  # ⚠️ 强制转 list
        except Exception as e:
            error = True
            print(f"[text2traj parse failed]: eval failed: {e}")

    if not error and all(isinstance(item, (list, tuple)) and len(item) == dim for item in description_list):
        try:
            if len(description_list) == frame - 1:
                description_list = description_list + [description_list[-1]]  # ⚠️ tuple 无 append 改为拼接
            ...
            if len(description_list) == frame:
                ...
                error = True
                print(f"[text2traj parse failed]: length mismatch after adjustment: {len(description_list)} vs {frame}")
        except Exception as e:
            error = True
            print(f"[text2traj parse failed]: array conversion failed: {e}")
    else:
        if not error:
            print("[text2traj parse failed]: not all elements have correct type/length")
        error = True

    if not error:
        return traj
    else:
        print(f"[text2traj parse failed]: final error on input: {description}")
        return None
